...
hbs.registerHelper("mutualFollow", function (options) {
  ...
  const isUserFollowing = userFollower.followeds.some((follow) => follow.followed._id.toString() === userFollowed._id.toString())
  const isUserFollowed = userFollower.followers.some((follow) => follow.follower._id.toString() === userFollowed._id.toString())
